...
use constant DEBUG			=> 1;#turns off serial port
use constant AUTO			=> 1;
...
my @service_head = $dbd->get_service_head();
my %service = ();
foreach my $s (@service_head){
	print "\n'$s'";
	$service{$s}{amount} = $dbd->get_total_spent_on_service($s);
	#print " $s = ".$service{$s}{amount}."\n";
}
