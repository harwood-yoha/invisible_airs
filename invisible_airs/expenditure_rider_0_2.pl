...
if( WITH_SANDERS){
	use constant MAX_TIME_INTERVAL => 16000000; #prev 1600000
}else{
	use constant MAX_TIME_INTERVAL => 11000000; #prev 1100000 
}
...
sub fire_air_cannon {
	...
	my $sleep = $total_sleep_time / 10; #prev /10
	my $cnt = 1;
	do {
		usleep($sleep);	
...
sub close_valve_out {		
    $port->write("c") if ! DEBUG;#air-out pin 12 HIGH	
}

sub open_valve_out {

	$port->write("d") if ! DEBUG;#air-in pin 12 LOW
}
