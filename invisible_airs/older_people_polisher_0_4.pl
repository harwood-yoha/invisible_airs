...
use constant WITH_SANDERS		=> 1;#using sanders
use constant WITH_POTATO		=> 0;#using potato
use constant WITH_SEAT			=> 0;#using seat

use constant DISPLAY_PAUSE		=> 1000000;
use constant SERIAL_PORT		=> qw( /dev/ttyUSB0 /dev/ttyUSB1 /dev/ttyUSB2);
...
if( WITH_SANDERS){
	use constant MAX_TIME_INTERVAL => 30000000;
}else{
	use constant MAX_TIME_INTERVAL => 30000000; 
}
...
while( $k ne 'q') {
	...
	sleep(5); # to allow car battery charger time to refill
}
...
sub auto {
	...
	if(WITH_SANDERS){
		if ($alternate == 1) {
			trigger_sanders($display_ref->{pressure},$display_ref->{ln},1);
			$alternate = 0;
		} else {
			trigger_sanders($display_ref->{pressure},$display_ref->{ln},0);
			$alternate = 1;
		}
	}else{
...
sub manual {
	...
	while ($key ne 'q') {
		...
		$key_buf .= $key;
			...
			#	$scr->clrscr;
			if(WITH_SANDERS){
				trigger_sanders($display_ref->{pressure},$display_ref->{ln},$alternate);
...
sub trigger_sanders {
	...
	my ($psi,$ln,$rev) = @_; # what does $rev do,
	my $current_pressure = MIN_PRESSURE;
	my $total_sleep_time = psi_to_time($psi);	
	...
	foreach(@s_arr){
		#print "REV = $rev $_";
		fire_sander($_);
		sanders_time_to_screen ($sander_cycle,$_,$ln);
		switch_off_sander($_);
		usleep(SANDER_DELAY);
	}
	sleep(1);
...
sub fire_sander {
	my $sander = shift;
	my @index = ('e','g','i');
	print "ON Sander $sander ".$index[$sander]. "";
	$port->write($index[$sander]) if ! DEBUG;
}


sub switch_off_sander {
	my $sander = shift;
	my @index = ('f','h','j');
	print "OFF $sander " . $index[$sander]. " ";
	$port->write($index[$sander]) if ! DEBUG;
}
...
sub disp_sander_air {
	...
	$scr->normal();
	$scr->at(ROW_PRESSURE+($ln+=2), 
		($scr->cols()/2) - COL_PRESSURE)->clreol()->bold()->puts('PREPARING SANDER: '. $sander_num);
